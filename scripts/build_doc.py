...
def main() -> int:
    if len(sys.argv) != 7:
        raise SystemExit("usage: build_doc.py <doc_src> <diagram_snippets> <plot_snippets> <language_snippets> <example_snippets> <out>")

    doc_src = Path(sys.argv[1]).read_text()
    diagram_snippets = Path(sys.argv[2]).read_text()
    plot_snippets = Path(sys.argv[3]).read_text()
    language_snippets = Path(sys.argv[4]).read_text()
    example_snippets = Path(sys.argv[5]).read_text()
    ...
    Path(sys.argv[6]).write_text(built)
    return 0
